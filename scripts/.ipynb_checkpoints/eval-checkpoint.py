...
# --- UPDATED: Pass assets functionally ---
def evaluate_retrieval_profile(
    ...
    if retrieval_gold:
        ...
        _docs, _diagnostics = retrieve_documents(first_query, assets, profile_name)

    start = time.perf_counter()

    for item in retrieval_gold:
        query = item["query"]
        docs, diagnostics = retrieve_documents(query, assets, profile_name)

        # Deduplicate IDs while preserving order
        retrieved_ids = list(dict.fromkeys(d.get("chunk_id") for d in docs if d.get("chunk_id")))
...
def evaluate_abstention(
    abstention_gold: list[dict[str, Any]],
    profile_name: str,
    assets: tuple,
) -> dict[str, Any]:
    ...
    # Warmup
    if abstention_gold:
        _docs, _diag = retrieve_documents(abstention_gold[0]["question"], assets, profile_name)

    per_query = []
    start = time.perf_counter()

    for item in abstention_gold:
        query = item["question"]
        _docs, diagnostics = retrieve_documents(query, assets, profile_name)

        top_rerank = diagnostics.get("top_rerank_score")
        top_fused = diagnostics.get("top_weighted_rrf_score")

        if profile_name == "hybrid_rerank":
            gate_score = top_rerank
            gate_threshold = NO_ANSWER_MIN_RERANK_SCORE
            gate_field = "top_rerank_score"
        elif profile_name == "hybrid":
            gate_score = top_fused
            gate_threshold = NO_ANSWER_MIN_FUSED_SCORE
            gate_field = "top_weighted_rrf_score"
        else:
            gate_score = None
            gate_threshold = None
            gate_field = None

        if gate_score is None or gate_threshold is None:
            abstained_correctly = None
        else:
            abstained_correctly = float(gate_score) < gate_threshold

        per_query.append({
            "id": item.get("id"),
            "question": query,
            "category": item.get("category"),
            "expected_decision": item.get("expected_decision", "abstained"),
            "gate_field": gate_field,
            "gate_score": gate_score,
            "gate_threshold": gate_threshold,
            "abstained_correctly": abstained_correctly,
            "top_rerank_score": top_rerank,
            "top_weighted_rrf_score": top_fused,
        })

    elapsed = time.perf_counter() - start

    applicable = [x for x in per_query if x["abstained_correctly"] is not None]
    correct = [x for x in applicable if x["abstained_correctly"]]

    aggregate = {
        "abstention_precision": (
            len(correct) / len(applicable) if applicable else None
        ),
        "num_correct": len(correct),
        "num_applicable": len(applicable),
        "num_total": len(per_query),
        "avg_latency_sec": (elapsed / len(per_query)) if per_query else None,
    }
...
def evaluate_generation(
    ...
    for item in qa_gold:
        ...
        start = time.perf_counter()
        # Answer question uses the functional paradigm
        ...
        # In case retrieve_documents wasn't directly accessible, rely on default metrics if missing
        ...
        records.append({
            ...
            "retrieved_ids": retrieved_ids,
            "retrieved_sources": [f"{c['source']}#p{c.get('page', 'Unknown')}" for c in result["retrieved_chunks"]],
            "retrieval_diagnostics": diagnostics,
...
def main() -> None:
    ...
    # --- UPDATED: Load functional assets once for the entire execution ---
    ...
    abstention_gold_path = EVAL_DIR / "abstention_gold.json"
    if abstention_gold_path.exists():
        abstention_gold = load_json(abstention_gold_path)
        abstention_results = [
            evaluate_abstention(abstention_gold, profile_name, assets)
            for profile_name in RETRIEVAL_PROFILES.keys()
        ]
    else:
        print(f"Note: {abstention_gold_path} not found; skipping abstention eval.")
        abstention_results = []
    ...
    if abstention_results:
        abstention_file = RESULTS_ABSTENTION_FILE
        if args.output_suffix:
            suffix = args.output_suffix.strip()
            abstention_file = abstention_file.with_name(
                f"{abstention_file.stem}_{suffix}{abstention_file.suffix}"
            )
        save_json(abstention_results, abstention_file)
        print(f"Saved abstention results → {abstention_file}")

        print("\nAbstention precision per profile:")
        for r in abstention_results:
            agg = r["aggregate"]
            precision = agg["abstention_precision"]
            if precision is not None:
                print(
                    f"  {r['profile_name']:15s} {precision:.2%} "
                    f"({agg['num_correct']}/{agg['num_applicable']})"
                )
            else:
                print(f"  {r['profile_name']:15s} N/A (no gate for this profile)") 
